...
if args.updatedata:
    ...
    items = ['items.json', 'items-base.json', 'magicvariants.json', 'vehicles.json', 'fluff-vehicles.json', 'backgrounds.json',
             'fluff-backgrounds.json', 'feats.json', 'races.json', 'fluff-races.json', 'books.json', 'adventures.json', 'optionalfeatures.json']
    ...
    try:
        ...
        for k, v in idx.items():
            ...
            with open(os.path.join(bestiarydir, v), 'wb') as f:
                f.write(req.content)
                f.close()
        print("Downloading bestiary fluff index:",
              "/bestiary/fluff-index.json")
...
for file in args.inputJSON:
    ...
    if os.path.isfile(os.path.split(file)[0] + "/fluff-" + os.path.split(file)[1]):
        if args.verbose:
            print("Fluff file found:", os.path.split(file)
                  [0] + "/fluff-" + os.path.split(file)[1])
        ...
        for m in d['monster']:
            ...
            for xmlmon in compendium.findall("./monster[name='{}']".format(re.sub(r'\'', '*', m['name']))):
                if args.verbose or args.showdupe:
                    print("{0} in {1} is duplicate entry for {2} from {3}".format(m['name'], utils.getFriendlySource(
                        m['source'], args), xmlmon.find('name').text, xmlmon.find('source').text if xmlmon.find('source') != None else '--'))
                mdupe += 1
    ...
    if 'vehicle' in d:
        for m in d['vehicle']:
            ...
            for xmlmon in compendium.findall("./monster[name='{}']".format(re.sub(r'\'', '*', m['name']))):
                if args.verbose or args.showdupe:
                    print("{0} in {1} is duplicate entry for {2} from {3}".format(m['name'], utils.getFriendlySource(
                        m['source'], args), xmlmon.find('name').text, xmlmon.find('source').text))
                mdupe += 1
            if fluff is not None and 'vehicleFluff' in fluff:
                if 'entries' in m:
                    m['entries'] += utils.appendFluff(fluff,
                                                      m['name'], 'vehicleFluff', args.nohtml)
                else:
                    m['entries'] = utils.appendFluff(
                        fluff, m['name'], 'vehicleFluff', args.nohtml)
                if 'image' not in m:
                    m['image'] = utils.findFluffImage(
                        fluff, m['name'], 'vehicleFluff')
            if 'alignment' not in m:
                m['alignment'] = ['U']
            if m['vehicleType'] == "INFWAR":
                m['type'] = "vehicle ({:,d} lb.)".format(m['weight'])
                m['ac'] = ["{} (19 while motionless)".format(
                    19+utils.getAbilityMod(m["dex"]))]
                m['hp'] = {"special": "{} (damage threshold {}, mishap threshold {})".format(
                    m['hp']['hp'], m['hp']['dt'], m['hp']['mt'])}
                if 'action' not in m:
                ...
                if 'action' not in m:
                    ...
                    m['action'] = [
                        {"type": "entries", "entries": copy.deepcopy(m['action'])}]
                if 'hull' in m:
                    if args.nohtml:
                        m['trait'].append({
                            "name": "Hull:",
                            ...
                            "entries": [
                                ...
                                "Hit Points: {}{}{}".format(m['hull']['hp'],
                                                            " (damage threshold {})".format(
                                                                m['hull']['dt']) if 'dt' in m['hull'] else '',
                    ...
                    for c in m['movement']:
                        ...
                        if 'locomotion' in c:
                            for l in c['locomotion']:
                                if args.nohtml:
                                    trait['entries'].append("Locomotion ({}): ".format(
                                        l['mode']) + "\n".join(l['entries']))
                                else:
                                    trait['entries'].append(
                                        "<i>Locomotion ({}):</i> ".format(l['mode']) + "\n".join(l['entries']))
                        if 'entries' in c:
                            trait['entries'] += c['entries']
                        m['trait'].append(trait)
                if 'weapon' in m:
                ...
                m['speed'] = "{} miles per hour ({} miles per day)".format(
                    m['pace'], m['pace']*24)
            else:
    ...
    if 'spell' in d:
        for m in d['spell']:
            ...
            for xmlmon in compendium.findall("./spell[name='{}']".format(re.sub(r'\'', '*', m['name']))):
                if args.verbose or args.showdupe:
                    print("Found duplicate entry for {} from {}".format(m['name'], xmlmon.find(
                        'source').text if xmlmon.find('source') != None else '--'))
                sdupe += 1
    ...
    if 'background' in d:
        for m in d['background']:
            ...
            if fluff is not None and 'backgroundFluff' in fluff:
                if 'entries' in m:
                    m['entries'] = utils.appendFluff(
                        fluff, m['name'], 'backgroundFluff', args.nohtml) + m['entries']
                else:
                    m['entries'] = utils.appendFluff(
                        fluff, m['name'], 'backgroundFluff', args.nohtml)
                if 'image' not in m:
                    m['image'] = utils.findFluffImage(
                        fluff, m['name'], 'backgroundFluff')
            if ignoreError:
                try:
    ...
    if 'race' in d:
        for race in d['race']:
            ...
            if fluff is not None and 'race' in fluff:
                if 'entries' in m:
                    m['entries'] += utils.appendFluff(fluff,
                                                      m['name'], 'raceFluff', args.nohtml)
                else:
                    m['entries'] = utils.appendFluff(
                        fluff, m['name'], 'raceFluff', args.nohtml)
                if 'image' not in m:
                    m['image'] = utils.findFluffImage(
                        fluff, m['name'], 'raceFluff')
            if 'subraces' in m:
                for sub in m['subraces']:
                    ...
                    if args.skipua:
                        if 'source' in sub and sub['source'].startswith('UA'):
                            if args.verbose:
                                print("Skipping UA Content: Subrace ",
                                      sub['name'] if 'name' in sub else m['name'])
                            continue
                    if args.onlyofficial:
                        if 'source' in sub and sub['source'] not in args.onlyofficial:
                            if args.verbose:
                                print("Skipping unoffical content: {} from {}".format(
                                    sub['name'], utils.getFriendlySource(sub['source'], args)))
                            continue
                    sr = copy.deepcopy(m)
                    ...
                    for xmlmon in compendium.findall("./race[name='{}']".format(re.sub(r'\'', '*', sr['name']))):
                        if args.verbose or args.showdupe:
                            print(
                                "Found duplicate entry for {}".format(sr['name']))
                        rdupe += 1
                    if ignoreError:
                        try:
            ...
            for v in mv['variant']:
                ...
                for req in v['requires']:
                    for m in d['baseitem']:
                        ...
                        if args.srd:
                            req["srd"] = True
                        itemMatch = all([k in m and m[k] == req[k]
                                         for k in req])
                        if 'excludes' in v:
                            for ex in v['excludes']:
                                if type(v['excludes'][ex]) == list:
                                    if any(sub in m[ex] for sub in v['excludes'][ex]):
                                        itemMatch = False
    ...
    if 'optionalfeature' in d:
        for m in d['optionalfeature']:
            ...
            for fType in fTypes:
                ...
                if fType == "AF":
                    m['name'] = "Formula: " + m['name']
                elif fType == "AI":
                    m['name'] = "Infusion: " + m['name']
                elif fType == "AS":
                    m['name'] = "Arcane Shot: " + m['name']
                elif fType == "ED":
                    m['name'] = "Elemental Discipline: " + m['name']
                elif fType == "EI":
                    m['name'] = "Invocation: " + m['name']
                elif fType == "MV":
                    m['name'] = "Maneuver: " + m['name']
                elif fType == "OR":
                    m['name'] = "Resonant: " + m['name']
                elif fType == "PB":
                    m['name'] = "Pact Boon: " + m['name']
                elif fType == "RN":
                    m['name'] = "Rune: " + m['name']
...
if args.combinedoutput:
    ...
    if args.combinedoutput.endswith(".compendium"):
        if mwins == 0 and swins == 0 and iwins == 0:
            ...
            tree.write(os.path.join(args.tempdir, "compendium.xml"),
                       xml_declaration=True, short_empty_elements=False, encoding='utf-8')
            zipfile = shutil.make_archive(
                args.combinedoutput, "zip", args.tempdir)
        ...
        if mwins == 0 and swins == 0 and iwins == 0 and fwins == 0 and bwins == 0 and rwins == 0 and cwins == 0 and \
            ...
            tree.write(args.combinedoutput, xml_declaration=True,
                       short_empty_elements=False, encoding='utf-8')
